...
# Local Paths
# ============================
# We point directly to the local files. 
# Ensure these exist in your project folder structure.
HOLDOUT_ENGINEERED_PATH = Path("data/processed/feature_engineered_holdout.csv")
HOLDOUT_META_PATH = Path("data/processed/cleaned_holdout.csv")
...
def load_data():
    ...
    if not HOLDOUT_ENGINEERED_PATH.exists() or not HOLDOUT_META_PATH.exists():
        st.error(f"❌ Data files not found at {HOLDOUT_ENGINEERED_PATH} or {HOLDOUT_META_PATH}")
        st.stop()
...
if st.button("Show Predictions 🚀"):
    ...
    if len(idx) == 0:
        ...
        try:
            ...
            # Yearly Trend Chart
            # ============================
            # (Logic remains identical, just purely strictly local execution)
            if region == "All":
